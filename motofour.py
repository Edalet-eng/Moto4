...
with interface:
    ...
    selected = option_menu( menu_title=None, 
                           options=['Proqnozlaşdırmaq','Məsləhətçi'], 
                           icons=['car-front-fill','lightbulb-fill'],
                           orientation='horizontal')
    
    if selected == 'Proqnozlaşdırmaq':
    
        st.header('Avtomobilin özəlliklərini daxil et')
...
#məsləhətçi bot            
    if selected == 'Məsləhətçi':
        ...
        with st.form("chat_form"):
            ...
            car_info_button = st.form_submit_button("Daxil edilən avtomobil haqqında ümumi məlumat al")
        if submit_button:
